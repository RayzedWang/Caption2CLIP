...
def calcul_loss(scores, size, margin, loss_type="mse", max_violation=False, labels=None, is_sum=True):
    diagonal = scores.diag().view(size, 1)
    if labels is not None:
        hashed_labels = list(map(lambda x: hash(x), labels))
        labels_tensor = torch.tensor(hashed_labels)
        unique_labels, label_indices = torch.unique(labels_tensor, return_inverse=True)
        label_mask = label_indices.unsqueeze(1) == label_indices.unsqueeze(0)
    ...
    if labels is not None:
        label_mask = label_mask.to(cost_s.device)
    ...
    if is_sum:
        ...
        return (cost_s.sum() + cost_im.sum())/size/2

def calcul_Aligin_loss(detail_feature, text_feature, margin, is_sum = True):
    cos_sim = F.cosine_similarity(detail_feature, text_feature)
    cost_s = (1 - cos_sim - margin).clamp(min=0)
    if is_sum:
        return cost_s.sum()
    else:
        return cost_s.mean()
...
def shard_dis_Detail_FGW_Version(images, captions, detail, model, words, shard_size=96, lengths=None,stage=True):
    """compute image-caption pairwise distance during validation and test"""
    n_im_shard = (len(images) - 1) // shard_size + 1
    n_cap_shard = (len(captions) - 1) // shard_size + 1
    d = np.zeros((len(images), len(captions)))
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            with torch.no_grad():
                ...
                det = detail[im_start:im_end]
                word = words[im_start:im_end]
                l = lengths[cap_start:cap_end]
                sim, _ = model(im, text = s, detail_text = det, word=word, device='cuda',stage=stage)
                sim = sim.squeeze()
                d[im_start:im_end, cap_start:cap_end] = sim.data.cpu().numpy()
...
def save_checkpoint(state, is_best, filename, prefix='', model_name = None):
    ...
    while tries:
        try:
            if lastok is False:
                torch.save(state, prefix + model_name + '_last.pth')
                lastok = True
...
def adjust_learning_rate_detail(options, optimizer, epoch):
    """Sets the learning rate to the initial LR
       decayed by 10 every 30 epochs"""
    for param_group in optimizer.param_groups:
        lr = param_group['lr']
        is_clip_param = any("clipmodel" in name for name in param_group['param_names'])
