...
class VisionTransformer(nn.Module):
    ...
    def emb_img_full(self, x: torch.Tensor, noproj: bool = False):
...
class CLIP(nn.Module):
    ...
    def encode_image_full(self, image, noproj=False):
        return self.visual.emb_img_full(image.type(self.dtype),noproj)
    ...
    def encode_text_full_andfeature(self, text, noproj=False):
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        ...
        if noproj==True:
            x = x
            feature = self.ln_final(x).type(self.dtype)[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection
        else:
            x = self.ln_final(x).type(self.dtype) @ self.text_projection
            feature = x[torch.arange(x.shape[0]), text.argmax(dim=-1)].clone()
        # x.shape = [batch_size, n_ctx, transformer.width]
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        # x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection
    ...
    def embed_text(self, text):
        ...
        is_eos_id = (text == 49407)
        text_mask_excluding_eos = is_eos_id.cumsum(dim = -1) == 0
        text_mask = F.pad(text_mask_excluding_eos, (1, -1), value = True)
        text_mask = text_mask & (text != 0)
        #assert not self.cleared

        text_embed,text_feature = self.encode_text_full_andfeature(text)
        text_encodings = text_embed
        text_encodings = text_encodings.masked_fill(~text_mask[..., None], 0.)

        return text_encodings, text_feature

    def embed_text_EOSid(self, text):
        #text = text[..., :self.max_text_len]

        text_embed,text_feature = self.encode_text_full_andfeature(text)
        id = text.argmax(dim=-1)
        return text_embed, text_feature, id
